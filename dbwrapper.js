...
const DBWrapper = {
	...
	userExists: (user_number) => {
		return fs
			...
			.filter(foldname => { return foldname == user_number; })
			.length > 0;
	},
	...
	userAdd: (user_number) => {
		if (DBWrapper.userExists(user_number)) {
			console.warn(`Attempted to create user ${user_number}, who already exists`);
			return;
		}
	...
	dayRatingGetAll: (user_number) => {
		if (!DBWrapper.userExists(user_number)) {
			console.error(`Attempted adding day rating to non-existent user ${user_number}`)
			return;
		}

		// Second test clause for the actual file
		const filesinfolder = fs.readdirSync(path.join(__dirname, 'db', String(user_number)));

		const filepath = path.join(__dirname, 'db', String(user_number), 'dayratings.json');
